...
function sanitizeCustomTopics(input) {
  if (!Array.isArray(input)) return [];
  return [...new Set(input.map((t) => String(t || '').trim()).filter(Boolean).map((t) => t.slice(0, 40)))].slice(0, 5);
}
...
async function planDrawing({ destination, features, customTopics, weather, family }) {
  const featureText = features.length ? features.map((f) => `- ${f}`).join('\n') : '(지정 안 함)';
  const customText = customTopics.length ? customTopics.map((t) => `- ${t}`).join('\n') : '(지정 안 함)';
...
async function handleTravel(body, options = {}) {
  ...
  const customTopics = sanitizeCustomTopics(body?.customTopics);
  ...
  const plan = await planDrawing({ destination, features, customTopics, weather, family });
  if (plan.error) {
