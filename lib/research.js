...
async function handleResearch(body, options = {}) {
  ...
  if (saveResult.status !== 201) {
    report.saveError = saveResult.body?.error || '보고서 저장에 실패했습니다.';
    return { status: 502, body: response };
  }
  ...
  if (body?.sendKakao) {
    ...
    report.kakao = { sent: false, reason: 'skipped' };
  }

  return { status: 200, body: response, setCookie: options.setCookie };
}
