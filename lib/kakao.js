...
function hasClientSecretConfigured() {
  const secret = getClientSecret();
  return Boolean(secret && !secret.includes('your-kakao'));
}
...
function explainTokenError(rawMessage) {
  const message = String(rawMessage || '');
  const lower = message.toLowerCase();

  if (!lower.includes('bad client credentials')) {
    return message || '카카오 토큰 발급에 실패했습니다.';
  }

  if (!hasClientSecretConfigured()) {
    return (
      '카카오 토큰 발급에 실패했습니다. Vercel(또는 서버) 환경변수에 KAKAO_CLIENT_SECRET이 없습니다. ' +
      '카카오 개발자 콘솔에서 Client Secret을 사용 중이면 npm run sync:vercel-env 로 동기화하거나 Vercel 대시보드에 직접 추가한 뒤 재배포하세요.'
    );
  }

  return (
    '카카오 토큰 발급에 실패했습니다. REST API 키와 Client Secret이 카카오 개발자 콘솔의 앱과 일치하는지 확인해 주세요. ' +
    `(원인: ${message})`
  );
}
...
function buildReportUrl(reportId) {
  return `${getAppBaseUrl()}/report?id=${encodeURIComponent(reportId)}`;
}
...
async function requestToken(bodyParams) {
  ...
  if (!response.ok) {
    const raw = data.error_description || data.error || '카카오 토큰 발급에 실패했습니다.';
...
function buildMemoTemplate({ title, summary, reportUrl, imageUrl }) {
  const safeTitle = truncate(title, 70) || '방산 리서치 보고서';

  if (imageUrl) {
    const safeSummary = truncate(summary, 180) || '(설명 없음)';
    return {
      object_type: 'feed',
      content: {
        title: safeTitle,
  ...
  const safeSummary = truncate(summary, 110) || '(요약 없음)';
  const text = truncate(`${safeTitle}\n\n${safeSummary}`, 200);
...
function getConnectionStatus(cookieHeader) {
  ...
  return {
    ...
    hasClientSecret: hasClientSecretConfigured(),
...
async function trySendReportMemo(cookieHeader, { title, summary, reportId, imageUrl }) {
  if (!isConfigured()) {
  ...
  try {
    ...
    const template = buildMemoTemplate({ title, summary, reportUrl, imageUrl });
