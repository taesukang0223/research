...
const MAX_FILE_BYTES = 4 * 1024 * 1024; // Vercel 요청 본문 한도(~4.5MB) 고려
...
function validateMimeType(mimeType) {
  ...
  if (!ALLOWED_MIME_TYPES.has(normalized)) {
    return {
      error: `지원하지 않는 파일 형식입니다. 허용: PDF, JPEG/PNG/WebP/GIF, MP3/WAV/AAC/OGG/FLAC`,
    };
  }
...
function parseMultipart(req) {
  return new Promise((resolve, reject) => {
    ...
    busboy.on('file', (_name, stream, info) => {
      ...
      stream.on('limit', () => reject(new Error(`파일 크기는 ${MAX_FILE_BYTES / (1024 * 1024)}MB 이하여야 합니다.`)));
      stream.on('end', () => {
...
async function handleAnalyzeFile(input) {
  ...
  try {
    ...
    if (uploaded.state !== 'ACTIVE') {
      const waitResult = await waitForFileActive(geminiFileName);
      if (waitResult.error) {
...
async function handleAnalyzeFileRequest(req) {
  ...
  return handleAnalyzeFile(parsed.data);
}
